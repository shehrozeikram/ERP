...
// Helper function to find or create reference (improved version)
const findOrCreateReference = async (Model, field, value, createData = {}) => {
  ...
  try {
    ...
    if (!ref) {
      ...
      const newData = { [field]: value, isActive: true, ...createData };
      
      // Add code field for Department
      if (Model.modelName === 'Department' && !newData.code) {
        newData.code = value.toUpperCase().replace(/\s+/g, '').substring(0, 10);
      }
      
      ref = await Model.create(newData);
...
const importEmployeesFromExcel = async () => {
  try {
    console.log('🚀 Starting improved employee import from Master_File_Aug_2025.xlsx...');
    ...
    for (let i = 1; i < data.length; i++) { // Skip header row
      ...
      try {
        ...
        const departmentId = await findOrCreateReference(Department, 'name', employeeData['Department']);
        const designationId = await findOrCreateReference(Designation, 'title', employeeData['Designation']);
        const bankId = await findOrCreateReference(Bank, 'name', employeeData['Bank']);
        const projectId = await findOrCreateReference(Project, 'name', employeeData['Project']);
        const locationId = await findOrCreateReference(Location, 'name', employeeData['Location']);
        ...
        if (employeeData['Section']) {
          try {
            ...
            if (!section) {
              ...
              await section.save();
              console.log(`✅ Created new Section: ${employeeData['Section']}`);
            }
        ...
        if (successCount % 50 === 0) {
          console.log(`📝 Processed ${successCount} employees...`);
        }
        
      } catch (error) {
        errorCount++;
        console.error(`❌ Error processing row ${i + 1}:`, error.message);
        console.error('Employee ID:', employeeData['ID']);
      }
    }
    ...
    console.log(`✅ All employees imported with unique employeeId constraint`);
