/**
 * CEO office fixed assets import (handwritten FAR logs, June 2024).
 * Used by API route and seed script.
 */

const FixedAsset = require('../models/finance/FixedAsset');

const CEO_OFFICE_IMPORT_TAG = 'FAR log import — CEO office (Jun 2024)';
...
const toAssetDoc = (row) => {
  ...
  const description = [CEO_OFFICE_IMPORT_TAG, remarks].filter(Boolean).join(' — ');
  return {
    name: clean(row.name),
...
const importTagRegex = () => CEO_OFFICE_IMPORT_TAG.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');

async function removeLegacyCombinedSofa() {
  const legacySofa = await FixedAsset.findOne({
    name: 'Sofa chair (5)',
    description: { $regex: importTagRegex() }
  }).select('_id assetNumber').lean();

  if (!legacySofa) return null;
  await FixedAsset.deleteOne({ _id: legacySofa._id });
  return legacySofa;
}

async function importCeoOfficeFixedAssets({ userId } = {}) {
  const legacyRemoved = await removeLegacyCombinedSofa();
  const createdRows = [];
  const skippedRows = [];
  ...
  for (const row of ASSETS) {
    ...
    const existing = await FixedAsset.findOne({
      ...
      name: clean(row.name),
      location: buildLocation(row.floor, row.room)
    }).select('_id assetNumber name category').lean();
  ...
  return {
    importTag: CEO_OFFICE_IMPORT_TAG,
    totalInSet: ASSETS.length,
    created: createdRows.length,
    skipped: skippedRows.length,
    legacyRemoved: legacyRemoved
      ? { assetNumber: legacyRemoved.assetNumber, name: 'Sofa chair (5)' }
      : null,
    createdRows,
    skippedRows
  };
...
module.exports = {
  CEO_OFFICE_IMPORT_TAG,
  CEO_OFFICE_ASSET_COUNT: ASSETS.length,
  importCeoOfficeFixedAssets
};
