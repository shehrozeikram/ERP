...
async function fixAhsanComplete() {
  try {
    console.log('🔧 Fixing Muhammad Ahsan (5742) - COMPLETE...');
    ...
    allAttendance.slice(0, 7).forEach(a => {
    ...
    for (const attendance of allAttendance) {
      ...
      if (expected) {
        ...
        if (expected.checkIn) {
          ...
          const utcTime = new Date(checkInDate.getTime() - pakistanOffset);
        ...
        if (expected.checkOut) {
          ...
          // Adjust for timezone (convert to UTC)
          const pakistanOffset = 5 * 60 * 60 * 1000;
          const utcTime = new Date(checkOutDate.getTime() - pakistanOffset);
