...
/**
 * Fix Historical Attendance Times (Corrected)
 * 
 * This script fixes historical attendance data by correcting timezone issues properly.
 */
...
async function fixHistoricalTimesCorrect() {
  try {
    console.log('🔧 Fixing historical attendance times (corrected)...');
      ...
      try {
        ...
        if (attendance.checkIn && attendance.checkIn.time) {
          ...
          if (currentHour >= 13 && currentHour <= 18) {
            // Add 5 hours to convert to Pakistan time (the time was stored as UTC but should be local)
