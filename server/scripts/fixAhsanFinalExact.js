...
async function fixAhsanFinalExact() {
  try {
    console.log('🔧 Fixing Muhammad Ahsan (5742) - FINAL EXACT...');
    ...
    for (const attendance of allAttendance) {
      ...
      if (expected) {
        ...
        if (expected.checkIn) {
          ...
          // Use the date from the attendance record and set the exact time
          const checkInDate = new Date(attendance.date);
          checkInDate.setUTCHours(hours, minutes, seconds, 0);
        ...
        if (expected.checkOut) {
          ...
          checkOutDate.setUTCHours(hours, minutes, seconds, 0);
