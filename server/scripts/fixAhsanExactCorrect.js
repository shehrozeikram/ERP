...
async function fixAhsanExactCorrect() {
  try {
    console.log('🔧 Fixing Muhammad Ahsan (5742) - EXACT CORRECT...');
    ...
    for (const attendance of allAttendance) {
      ...
      if (expected) {
        ...
        if (expected.checkIn) {
          ...
          // Create a new date using the exact time string in Pakistan timezone
          const checkInDate = new Date(`${dateKey}T${expected.checkIn}:00+05:00`);
        ...
        if (expected.checkOut) {
          ...
          // Create the time in Pakistan timezone (exact as from ZKTeco device)
          const checkOutDate = new Date(`${dateKey}T${expected.checkOut}:00+05:00`);
