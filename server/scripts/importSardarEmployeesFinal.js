...
class SardarEmployeeImporterFinal {
  constructor() {
    this.stats = {
      total: 0,
      created: 0,
  ...
  async resolvePosition(positionName, departmentId) {
    if (!positionName || !departmentId) return null;
    
    try {
      // Try to find existing position in the same department
      ...
      if (!position) {
        ...
        position = new Position({
          title: positionName,
          code: uniqueCode,
          department: departmentId, // This is the key fix!
    ...
    try {
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        try {
          ...
          employeeData.isActive = true; // Set as active
          employeeData.employmentStatus = 'Active'; // Set employment status
          ...
          // Resolve references - Department first, then Position
          let departmentId = null;
          if (employeeData.departmentName) {
            departmentId = await this.resolveDepartment(employeeData.departmentName);
          ...
          if (!departmentId) {
            ...
            departmentId = await this.resolveDepartment('General');
            employeeData.department = departmentId;
          }
          
          if (employeeData.designationName) {
            employeeData.position = await this.resolvePosition(employeeData.designationName, departmentId);
            delete employeeData.designationName;
          }
          
          if (!employeeData.position) {
            // Create a default position if none exists
            employeeData.position = await this.resolvePosition('Employee', departmentId);
          }
...
// Main execution
async function main() {
  const importer = new SardarEmployeeImporterFinal();
...
module.exports = SardarEmployeeImporterFinal; 