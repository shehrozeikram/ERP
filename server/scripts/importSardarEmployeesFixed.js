...
class SardarEmployeeImporterFixed {
  constructor() {
    this.stats = {
      total: 0,
      created: 0,
  ...
  mapExcelRowToEmployee(headers, row) {
    ...
    // Employment details
    employee.employmentType = 'Full-time'; // Fixed enum value
    employee.employmentStatus = 'Active';
    employee.workLocation = this.cleanValue(rowData['Location']);
    employee.shift = 'Day';
    employee.workingDays = 6; // Default for Sardar
    ...
    try {
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        try {
          ...
          // Use designation as position if no separate position field
          if (employeeData.designation && !employeeData.position) {
            employeeData.position = employeeData.designation;
          }
...
// Main execution
async function main() {
  const importer = new SardarEmployeeImporterFixed();
...
module.exports = SardarEmployeeImporterFixed; 