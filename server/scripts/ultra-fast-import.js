...
const getOrCreateEntity = async (Model, data, uniqueField, codeField = null) => {
  try {
    let entity = await Model.findOne({ [uniqueField]: data });
    if (!entity) {
      const entityData = { [uniqueField]: data, isActive: true };
      if (codeField) {
        const shortCode = data.toUpperCase().replace(/\s+/g, '').substring(0, 10);
        entityData[codeField] = shortCode;
      }
...
const importEmployeesFromExcel = async () => {
  try {
    console.log('🚀 Starting ULTRA FAST employee import from Excel...');
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      // Get or create entities quickly
      const departmentId = await getOrCreateEntity(Department, employeeData['Department'], 'name', 'code');
      const designationId = await getOrCreateEntity(Designation, employeeData['Designation'], 'title');
      const bankId = await getOrCreateEntity(Bank, employeeData['Bank'], 'name');
      const projectId = await getOrCreateEntity(Project, employeeData['Project'], 'name');
      const locationId = await getOrCreateEntity(Location, employeeData['Location'], 'name');
      
      // Create section with department reference
      let sectionId = null;
      if (employeeData['Section']) {
        try {
          let section = await Section.findOne({ name: employeeData['Section'] });
          if (!section) {
            section = new Section({
              name: employeeData['Section'],
      ...
      if (employees.length >= batchSize) {
        console.log(`📝 Processing batch ${Math.ceil(processed / batchSize)} - ${employees.length} employees...`);
        try {
    ...
    if (employees.length > 0) {
      ...
      try {
        await Employee.insertMany(employees, { ordered: false });
        console.log(`✅ Final batch completed successfully`);
      } catch (error) {
        console.error(`❌ Final batch error:`, error.message);
      }
    }
    
    console.log('\n🎉 ULTRA FAST Employee import completed successfully!');
    console.log(`✅ All ${processed} employees imported with unique email addresses`);
