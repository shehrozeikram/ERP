...
const province = db.provinces.findOne({
  country: country._id,
  name: { $regex: /azad.*kashmir|azad jammu.*kashmir/i },
  isActive: { $ne: false }
});

if (!province) {
  throw new Error('Azad Kashmir province record not found in provinces collection');
}
...
function nextCode() {
  while (true) {
    const code = `AJK${String(codeCounter).padStart(3, '0')}`;
    codeCounter += 1;
...
for (const cityName of ajkCities) {
  const key = cityName.trim().toLowerCase();
  const present = existingNames.get(key);
  if (present) {
    exists += 1;
    // Ensure previously soft-deleted records are activated.
