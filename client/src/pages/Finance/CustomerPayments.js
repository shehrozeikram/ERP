...
export default function CustomerPayments() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await api.get('/finance/customer-payments', { params });
      setData(res.data.data);
    } catch (e) {
      setError(e.response?.data?.message || 'Failed to load customer payments');
    } finally {
      setLoading(false);
    }
  }, [filters]);
    ...
    <Box sx={{ p: 3 }}>
      ...
      <Grid container spacing={2} mb={3}>
        <Grid item xs={12} sm={4}>
          <Card sx={{ background: 'linear-gradient(135deg, #e8f5e9 0%, #c8e6c9 100%)', borderRadius: 2 }}>
            <CardContent>
              <Box display="flex" alignItems="center" gap={1} mb={0.5}>
                <WalletIcon color="success" fontSize="small" />
                <Typography variant="body2" color="text.secondary">Total Receipts</Typography>
              </Box>
              <Typography variant="h5" fontWeight={800} color="success.dark">{fmt(data.totalAmount)}</Typography>
      ...
      <Paper variant="outlined" sx={{ p: 2, mb: 2 }}>
        <Grid container spacing={2} alignItems="center">
          <Grid item xs={12} sm={4}>
            <TextField
              fullWidth size="small" label="Search customer / invoice / reference"
          ...
          <Table size="small">
            <TableHead>
              <TableRow sx={{ bgcolor: 'primary.main' }}>
                {['Payment Date', 'Invoice #', 'Customer', 'Method', 'Reference', 'Amount'].map(h => (
            ...
            <TableBody>
              {data.payments.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={6} align="center" sx={{ py: 4, color: 'text.disabled' }}>
                    No customer payments found for selected criteria
                  </TableCell>
                </TableRow>
              ) : (
                data.payments.map((p, i) => (
                  <TableRow key={p._id || i} hover sx={{ '&:nth-of-type(even)': { bgcolor: 'grey.50' } }}>
                    ...
                    <TableCell>
                      <Typography variant="body2" fontWeight={600} color="primary.main">{p.invoiceNumber}</Typography>
                    </TableCell>
                    <TableCell>
                      <Typography variant="body2" fontWeight={500}>{p.customerName}</Typography>
                      {p.customerEmail && <Typography variant="caption" color="text.secondary">{p.customerEmail}</Typography>}
                    </TableCell>
                    <TableCell>
                      <Chip
                        label={(p.paymentMethod || 'other').replace('_', ' ')}
                        size="small"
              ...
              {data.payments.length > 0 && (
                <TableRow sx={{ bgcolor: 'grey.100' }}>
                  <TableCell colSpan={5} sx={{ fontWeight: 700 }}>Total Receipts ({data.count} payments)</TableCell>
                  <TableCell sx={{ fontWeight: 800, color: 'success.dark' }}>{fmt(data.totalAmount)}</TableCell>
                </TableRow>
              )}
            </TableBody>
          </Table>
