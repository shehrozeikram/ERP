...
export default function VendorTrialBalancePanel({ supplierId, supplierName }) {
  ...
  const load = useCallback(async () => {
    ...
    setError('');
    try {
      const res = await api.get(`/finance/vendors/${supplierId}/trial-balance`, {
      ...
      setError(e.response?.data?.message || 'Failed to load vendor trial balance');
      setData(null);
    } finally {
      setLoading(false);
    }
  ...
  const loadLedger = useCallback(async (account) => {
    ...
    try {
      const res = await api.get(
        `/finance/vendors/${supplierId}/trial-balance/ledger/${account._id}`,
        { params: { fromDate, asOfDate } }
      );
  ...
  const loadVoucher = useCallback(async (ledgerRow) => {
    ...
    try {
      const res = await api.get(
        `/finance/vendors/${supplierId}/trial-balance/voucher/${jeId}`
      );
      ...
      {!data && !loading && (
        <Paper variant="outlined" sx={{ p: 4, textAlign: 'center' }}>
          <Typography color="text.secondary">
            Select dates and click Generate to view this vendor&apos;s trial balance.
          </Typography>
        </Paper>
      )}
      ...
      {data && !loading && (
        <>
          <Paper variant="outlined" sx={{ p: 1.5, mb: 1 }}>
            <Typography variant="body2" fontWeight={700}>Trial Balance — {supplierName}</Typography>
                ...
                {accounts.length === 0 && (
                  <TableRow>
                    <TableCell colSpan={7} align="center" sx={{ py: 3, color: 'text.secondary' }}>
                      No posted journal activity for this vendor in the selected period.
                    </TableCell>
                  </TableRow>
                )}
                {accounts.length > 0 && (
