...
const LandExchangeViewer = lazy(() => import('./LandExchangeViewer'));
...
const EXCHANGE_PATH = `${BASE_PATH}/exchange`;
...
const LandAcquisition = () => {
  ...
  const isExchangeTab = location.pathname.startsWith(EXCHANGE_PATH);
  ...
  const tabValue = isReportsTab ? 6 : isExchangeTab ? 5 : isPossessionTab ? 4 : isRegistryTab ? 3 : isMozaTab ? 2 : isMapsTab ? 1 : isDashboardTab ? 0 : false;
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      >
        <Tabs
          ...
          onChange={(_, value) => {
            ...
            if (value === 5) navigate(EXCHANGE_PATH);
            if (value === 6) navigate(REPORTS_PATH);
          }}
          sx={{
            px: 1,
            borderBottom: 1,
        ...
        >
          ...
          <Tab icon={<SwapHorizIcon fontSize="small" />} iconPosition="start" label="Land Exchange" />
      ...
      <Suspense fallback={
        ...
        {isExchangeTab && <LandExchangeViewer />}
