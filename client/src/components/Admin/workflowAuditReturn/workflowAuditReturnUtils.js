...
export function getWorkflowFeedbackObservations(doc) {
  ...
  if (Array.isArray(doc.auditRejectObservations) && doc.auditRejectObservations.length > 0) {
    return doc.auditRejectObservations.map((obs, idx) => ({
  ...
  if (Array.isArray(doc.auditObservations) && doc.auditObservations.length > 0) {
    return doc.auditObservations;
  }
  ...
  history.forEach((entry) => {
    ...
    // ". Observations:" block (avoid matching "Rejected with observations:" in the same line)
